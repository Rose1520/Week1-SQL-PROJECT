DROP DATABASE company_program;

CREATE DATABASE company_program;
...
SELECT hierarchy,jurisdiction
FROM jurisdiction_hierarchy
WHERE hierarchy ='region';

-- iii) No. of Individual Beneficiaries (1HH = 6 Individuals)
SELECT DISTINCT partner,sum(beneficiaries),beneficiary_type
...
SELECT sum(beneficiaries),beneficiary_type
FROM beneficiary_partner_data
WHERE beneficiary_type = 'Individuals';
...
SELECT partner,count(village)
FROM beneficiary_partner_data
GROUP BY partner;

-- No. of Districts reached by partner
